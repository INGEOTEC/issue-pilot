"""The hook that refuses /issue-pilot:issue-plan with nothing to plan.
...
class RequireRequest(unittest.TestCase):
    ...
    def test_a_bare_command_is_refused_and_told_why(self):
        for prompt in ("/issue-pilot:issue-plan", "/issue-pilot:issue-plan   ",
                       "  /issue-pilot:issue-plan\n"):
            with self.subTest(prompt=prompt):
                ...
                self.assertIn("/issue-pilot:issue-plan add a", out.stderr)

    def test_update_without_an_issue_number_is_refused_too(self):
        out = self.hook({"prompt": "/issue-pilot:issue-plan --update"})
        self.assertEqual(out.returncode, 2)

    def test_a_request_goes_through(self):
        for prompt in ("/issue-pilot:issue-plan add a --format flag to export",
                       "/issue-pilot:issue-plan --update 42",
                       "/issue-pilot:issue-plan --update #42",
    ...
    def test_every_other_prompt_is_none_of_its_business(self):
        for prompt in ("/issue-pilot:issues 1 2", "/issue-pilot:issue-planning",
                       "issue-plan", "hello", ""):
            with self.subTest(prompt=prompt):
